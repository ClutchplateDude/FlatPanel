...
//
// ASCOM CoverCalibrator driver for LutzFlatpanel
//
...
namespace ASCOM.LutzFlatpanel.CoverCalibrator
{
	//
	...
	//
	// Your driver's DeviceID is ASCOM.LutzFlatpanel.CoverCalibrator
	//
	// The COM Guid attribute sets the CLSID for ASCOM.LutzFlatpanel.CoverCalibrator
	// The COM ClassInterface/None attribute prevents an empty interface called _LutzFlatpanel from being created and used as the [default] interface
	//
	...
	/// </summary>
	[ComVisible(true)]
	[Guid("241ff3b6-79a3-43ef-9877-6eaaae7ebd55")]
	[ProgId("ASCOM.LutzFlatpanel.CoverCalibrator")]
	[ServedClassName("ASCOM CoverCalibrator Driver for LutzFlatpanel")] // Driver description that appears in the Chooser, customise as required
	...
	public class CoverCalibrator : ReferenceCountedObjectBase, ICoverCalibratorV2, IDisposable
	{
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="LutzFlatpanel"/> class. Must be public to successfully register for COM.
		/// </summary>
		public CoverCalibrator()
		{
			try
			{
				...
				tl = new TraceLogger("", "LutzFlatpanel.Driver"); // Remove the leading ASCOM. from the ProgId because this will be added back by TraceLogger.
				SetTraceState();
			...
			catch (Exception ex)
			{
				...
				MessageBox.Show($"{ex.Message}", "Exception creating ASCOM.LutzFlatpanel.CoverCalibrator", MessageBoxButtons.OK, MessageBoxIcon.Error);
			}
		}
